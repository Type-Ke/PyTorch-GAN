...
class QualityFitness(torch.nn.Module):

    def __init__(self):
        super(QualityFitness, self).__init__()

    def forward(self, input, target):  
        return torch.mean(input)

class DiversityFitness(torch.nn.Module):

    def __init__(self):
        super(DiversityFitness, self).__init__()

    def forward(self, input, target):  
        return 0

# Loss functions
quality_fitness = QualityFitness()
diversity_fitness = DiversityFitness()
adversarial_loss_bce = torch.nn.BCELoss()
adversarial_loss_mse = torch.nn.MSELoss()
adversarial_loss_heu = HeuristicLoss()
adversarial_losses = [adversarial_loss_bce, adversarial_loss_mse, adversarial_loss_heu]
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        for g in range(3):
            ...
            if g == 0:
                adversarial_loss = adversarial_loss_bce
            elif g == 1 :
                adversarial_loss = adversarial_loss_mse
            else:
                adversarial_loss = adversarial_loss_heu
